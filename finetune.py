        ...
        if self.mlm:
            ...
            if self.tokenizer.pad_token_id == self.tokenizer.eos_token_id:
                if self.tokenizer.padding_side == "right":
                    ...
                    labels[:, idx] = self.tokenizer.eos_token_id # quick fix
                else:
                    labels[:, -1] = self.tokenizer.eos_token_id # what if string is cutoff?
...
def main():
    ...
    trainer = Trainer(
        ...
        data_collator=EosCollator(tokenizer, pad_to_multiple_of=1, mlm=False),
    )
